...
一个运行在终端里的 **AI 编程助手**：基于 **ReAct + Plan Mode 双模式**，让 LLM 自主完成从理解需求、读写代码、执行命令到验证提交的完整编程任务，兼容 **Anthropic / OpenAI** 双协议，并通过 **MCP** 扩展外部工具。

**要解决的问题**：大多数 Coding Agent 只有「对话 → 生成代码」的单循环，面对复杂任务容易失控、出错后重复犯错，且缺乏可观测性与自我改进能力。本项目用工程化的方式，把 Agent 从「能用」做到「可靠」。

**三个核心技术亮点**：



* **双工程体系保障可靠性** — Loop Engineering（Workflow 编排、断点恢复、预算控制、Cron 调度）+ Harness Engineering（完整性校验、审计日志、速率限制、指标采集），让长任务可控、可观测、可恢复

* **失败驱动的自进化闭环** — 从执行轨迹中采集失败模式 → 自动生成 Skill → 评估晋升 → 注入复用（6 阶段闭环）；同时从复杂成功任务中沉淀经验型 Skill，让 Agent 不再从零摸索

* **Multi-Agent 并行协作** — 子 Agent 分发（Fork / SubAgent）+ Team 团队协作（Coordinator 模式）+ Git worktree 任务隔离，多 Agent 并行执行互不干扰

**快速上手**：Python >= 3.11，安装依赖、配置 API Key 即可运行，约 5 分钟跑起来 —— 见下方[快速开始](#快速开始)。
...
从零跑起来只需 3 步：**① 安装依赖 → ② 配置 API Key → ③ 启动**，全程约 5 分钟。以下是完整步骤；Harness / 自进化等高级配置见对应章节。
